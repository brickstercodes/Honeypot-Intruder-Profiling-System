...
- **Attack classification** — 9 categories (brute force, recon, web probe, malware drop, bot, etc.) with smart context-aware fallback
- **Bot detection** — distinguishes attack frameworks (nuclei, hydra, sqlmap) from generic crawlers and headless browsers
- **Alerting** — dashboard, Discord webhook, Telegram bot, email (Resend)
- **Forensic chain viewer** — dashboard tab explaining and displaying the live SHA-256 hash chain with integrity verification
- **Forensic storage** — tamper-evident SHA-256 hash chain, AES-256-GCM encrypted at rest
- **Secure relay** — snapshot encrypted with AES-256-GCM and sent to a 3rd device; automatic retry (up to 2x), source self-destructs on confirmed receipt
- **Response actions** — block/unblock IP, terminate session, auto-block at configurable threshold (default 55)
...
**Flow:** attacker hits trap → engine logs session → `saveState` encrypts snapshot (AES-256-GCM) → POSTs to receiver (with up to 2 automatic retries on failure) → on HTTP 200, `snapshot.json` is deleted from source.

Receiver endpoints:
- `GET /relay/status` — list received snapshots
- `GET /relay/data/:filename` — read a snapshot

On network failure, retry delays are 1s and 2s. If all retries fail, `snapshot.json` is kept on disk so no data is lost.
...
| `AUTO_BLOCK_THRESHOLD` | Severity score to auto-block | `55` |
